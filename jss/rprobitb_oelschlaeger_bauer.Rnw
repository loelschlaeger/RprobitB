...
<<model-sim-mixture, fig=TRUE, height = 6, width = 6>>=
...
<<dirichlet process prior parameters>>=
delta <- 0.1
xi <- numeric(P_r)
D <- diag(P_r)
nu <- P_r + 2
...
<<dirichlet process initial values>>=
z <- rep(1, ncol(beta))
C <- 1
...
<<dp-example, fig=TRUE, height = 4, width = 8>>=
par(mfrow = c(1,2))
