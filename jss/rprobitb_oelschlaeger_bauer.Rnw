...
<<model-sim-class-seq, fig=TRUE, height = 3>>=
...
<<example 4 berserk create covariates, eval = FALSE>>=
...
<<example 4 berserk prepare data, eval = FALSE>>=
data <- prepare_data(
...
The estimated class sizes are as follows:\footnote{Per default, point estimates are obtained as the posterior means. However, the \fct{point\_estimates} function has the additional \code{FUN} argument for computing custom point estimates.}
...
<<model-berserk-mixture, fig=TRUE>>=
