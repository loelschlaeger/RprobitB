...
Let's prepare the *Train* data set of the [mlogit package](https://cran.r-project.org/package=mlogit) for estimation. You can call `help(mlogit::Train)` for details on the data set. We consider the covariates `price` (type 1), `time`, `comfort` and `change` (each of type 3), where we link `price` and `time` to random effects[^3]. We consider all alternatives, therefore `alternatives` does not have to be specified.
...
[^3]: Note that alternative specific constants are excluded here.
...
If we want to simulate the choices of `N` deciders in `T` choice occasions[^4] among `J` alternatives from our model formulation `form`, we have to call
...
- `distr`: A named list of number generation functions from which the covariates are drawn. Each element of `distr` must be of the form `"cov" = list("name" = "<name of the number generation function>", ...)`, where `cov` is the name of the covariate[^5] and `...` are required parameters for the number generation function. Covariates for which no distribution is specified are drawn from a standard normal distribution. Possible number generation functions are
...
[^4]: `T` can be either a positive number, representing a fixed number of choice occasions for each decision maker, or a vector of length `N`, i.e. a decision maker specific number of choice occasions.

[^5]: For a covariate `cov` of type 1 or 3, you can either choose `"name" = cov` (to draw the covariate for all alternatives from the same distribution) or `"name" = cov_alternative` (to draw the covariate for a specific alternative from a specific distribution).
...
Let us now simulate the choices of `N = 100` decision makers in `T = 10` choice occasions on the `J = 3` alternatives "car", "bus" and "train". We want `C = 2` true latent classes with equal weights and specific distributions[^6] for our covariates:
...
[^6]: Note that the `cost` covariate for all alternatives is drawn from the same distribution. Also note that since we did not specify a distribution for `travel_time_bus`, this covariate is drawn from a standard normal distribution.
