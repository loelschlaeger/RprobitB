...
library("ggplot2")
mixtures_color <- c("#c06c74", "#c06c84", "#c06c94")
total_mixture_color <- "#6c5b7b"
p <- ggplot2::ggplot(as.data.frame(mixture)) +
  ggplot2::theme_void() +
  ggplot2::geom_line(aes(x = x, y = C1), color = mixtures_color[1]) +
  ggplot2::geom_line(aes(x = x, y = C2), color = mixtures_color[2]) +
  ggplot2::geom_line(aes(x = x, y = C3), color = mixtures_color[3]) +
  ggplot2::geom_line(
    aes(x = x, y = joint),
    color = total_mixture_color,
    linewidth = 0.8,
    linetype = "dashed"
  )
...
# usethis::use_logo("sticker/RprobitB_sticker.png")
