...
By default, the model parameters are drawn randomly from the prior distributions specified in Section~\ref{subsec:prior_and_posterior}. Alternatively, they can be set via a named list for the function's \code{true\_parameter} argument. The list can contain
\begin{itemize}
...
<<coef-model-train, fig=TRUE, height = 2>>=
...
Via the \code{re} argument, we specify random effects for all but the price coefficient, which we again fix to \code{-1} for monetary interpretation:
...
<<load model, echo = FALSE, results = hide>>=
...
<<berserk-freq, echo = FALSE, results = hide, fig = TRUE>>=
library("ggplot2")
library("dplyr")
fr <- summarize(group_by(model_berserk$data$choice_data, player_id),
                freq = sum(berserk) / n())
dat <- cbind(class, freq = fr$freq)
dat$est <- factor(data$est)
ggplot(dat, aes(x = freq, colour = est, fill = est)) +
  geom_density(aes(y = ..density..), alpha = 0.2, bw = 0.05) +
  geom_rug(aes(y = 0), length = unit(0.03, "npc"), show.legend = FALSE,
           position = position_jitter(width = 0.001, height=0)) +
  coord_cartesian(xlim = c(0,1)) +
  scale_x_continuous(name = "Berserking frequency") +
  scale_y_continuous(name = "Density") +
  labs(fill = "estimated class", colour = "estimated class") +
