...
\title{RprobitB: Bayes Estimation of Probit Models in \proglang{R}}
\Plaintitle{RprobitB: Bayes Estimation of Probit Models in R}
\Shorttitle{RprobitB}

%% - \Abstract{} almost as usual
\Abstract{
\pkg{RprobitB} is an \proglang{R} package for Bayes estimation of multinomial probit models on discrete choice panel data. Its strength lies in modeling heterogenous choices, because it is capable to approximate any underlying mixing distributions through a mixture of normal distributions. The number of latent classes do not need to be prespecified but can be updated within the algorithm on a weight-based strategy. Furthermore, the package provides great numerical stability since it does not require initialization nor approximation or maximization of any likelihood function. In this paper, we show how to estimate (mixed) probit models using \pkg{RprobitB} and make a comparision to other \proglang{R} packages that estimate probit models.
}

%% - \Keywords{} with LaTeX markup, at least one required
%% - \Plainkeywords{} without LaTeX markup (if necessary)
%% - Should be comma-separated and in sentence case.
\Keywords{Mixed multinomial probit models, Bayesian analysis, unobserved heterogeneity}
\Plainkeywords{Mixed multinomial probit models, Bayesian analysis, unobserved heterogeneity}
...
In this paper, we explain the \prolang{R} package \pkg{RprobitB) (Oelschläger, Bauer, Büscher and Batram 2021), available from the Comprehensive R Archive Network (CRAN) at https://CRAN.R-project.org/package=RprobitB.
