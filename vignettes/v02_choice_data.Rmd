...
No choice model without choice data, so this vignette^[This vignette was build using R `r paste(R.Version()[6:7], collapse = ".")` with the {RprobitB} `r utils::packageVersion("RprobitB")` package.] provides a reference for data management in {RprobitB}. We use the [Train data set](https://cran.r-project.org/web/packages/mlogit/vignettes/c2.formula.data.html#wide-format) from the mlogit package [@mlogit] for illustration. 
...
- By default, alternative specific constants (ASCs)^[ASCs capture the average effect on utility of all factors that are not included in the model. Due to identifiability, we cannot estimate ASCs for all the alternatives. Therefore, they are added for all except for the last alternative.] are added to the model. They can be removed by adding `+ 0` in the second spot, e.g. `choice ~ A | B + 0`.
...
```{r summary Train, fig.dim = c(8, 6), out.width = "75%", fig.align = "center"}
summary(data)
plot(data)
```
...
```{r data plot by choice, fig.dim = c(8, 6), out.width = "75%", fig.align = "center"}
