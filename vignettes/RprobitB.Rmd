...
![The flowchart of {RprobitB}. The functions `check_form()` and `overview_effects()` help the user to verify the correctness of the data and model specification. `train_test()` allows to split the data into an estimation and a validation part. `transform_fit()` allows to change normalization of the model after a model has been fitted. ](flowchart.png){width=80%}
