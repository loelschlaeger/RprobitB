...
Let's prepare the *Train* data set of the [mlogit package](https://cran.r-project.org/package=mlogit) for estimation. We consider the covariates `price` (type 1), `time`, `comfort` and `change` (each of type 3), where we link `price` and `time` to random effects[^2]. We consider all alternatives, therefore `alternatives` does not have to be specified.
...
Let us now simulate the choices of `N = 100` decision makers in `T = 10` choice occasions on the `J = 3` alternatives "car", "bus" and "train". We want `C = 2` true latent classes with equal weights and specific distributions[^5] for our covariates:
