...
compute_choice_probabilities = function(X, parameter) {
  ...
  P_f = ifelse(is.na(alpha),0,length(alpha))
    ...
    for(c in 1:C){
      ...
      if(P_f>0){
        if(P_r>0){
          ...
          cov_matrix = delta(J,j) %*% ( X[,-(1:P_f)] %*% matrix(Omega[,c],P_r,P_r) %*% t(X[,-(1:P_f)]) + Sigma_full ) %*% t(delta(J,j))
        }
        if(P_r==0){
      ...
      if(P_f==0){
        if(P_r>0){
          ...
          cov_matrix = delta(J,j) %*% ( X %*% matrix(Omega[,c],P_r,P_r) %*% t(X) + Sigma_full ) %*% t(delta(J,j))
        }
        if(P_r==0){
          upper = rep(0,J-1)
          cov_matrix = delta(J,j) %*% Sigma_full %*% t(delta(J,j))
