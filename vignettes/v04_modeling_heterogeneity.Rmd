...
for (c in 1:C_true) {
  for (n in 1:N[c]) {
    beta <- cbind(beta, oeli::rmvnorm(mean = b_true[, c], Sigma = matrix(Omega_true[, c, drop = F], ncol = P_r)))
  }
}
...
for (r in 1:100) {
  dp <- update_classes_dp(
    beta, z, b, Omega, delta, xi, D, nu, Theta, identify_classes = TRUE
  )
  z <- dp$z
  b <- dp$b
  Omega <- dp$Omega
...
plot_class_allocation(beta, z, b, Omega, r = 100, perc = 0.95)
```
