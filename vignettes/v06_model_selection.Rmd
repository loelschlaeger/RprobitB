...
As a competing model, we consider explaining the choices only by the alternative's price, i.e. the model with the formula `choice ~ price | 0`:^[This model is also saved in {RprobitB} and can be accessed via `data("model_train_sparse", package = "RprobitB")`.]
...
The $\text{lppd}$ is approximated as follows. Let $$p_{si} = \Pr(y_i\mid \theta_s)$$ be the probability of observation $y_i$ given the $s$-th set $\theta_s$ of parameter samples from the posterior. Then
...
```{r waictrace, fig.align = "center", fig.width = "50%", fig.dim = c(6,6), message = FALSE}
